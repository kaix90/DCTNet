...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        if 'img' in results:
            ...
            while not (h%2 == 0 and h%4 == 0 and h%8 == 0):
                h -= 1
            results['img'] = results['img'][:h, :, :]
            while not (w%2 == 0 and w%4 == 0 and w%8 == 0):
                w -= 1
            results['img'] = results['img'][:h, :w, :]
...
class Average(object):
    def __call__(self, results):
        results['img'] = torch.mean(results['img'].data.float().view(results['img'].size(0), -1), dim=1)
        return results

@PIPELINES.register_module
class DynamicInput(object):
    def __init__(self, gate_ratio=1.0):
        self.gate_ratio = gate_ratio

    def __call__(self, results):
        results['dynamic_input'] = self.gate_ratio
        return results
